...
class FreeformService : Service(), ScreenListener.ScreenStateListener {
    ...
    // Pakai APP_SETTINGS_NAME yang sama dengan seluruh app
    private val sp by lazy {
        getSharedPreferences(com.sunshine.freeform.app.MiFreeform.APP_SETTINGS_NAME, MODE_PRIVATE)
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when (intent.action) {
            ACTION_START_INTENT -> {
                ...
                // PENTING: hanya cek kalau package bukan null dan bukan package kita sendiri
                ...
                // Cek batas maksimal floating window - pakai sp yang benar
                ...
                if (parcelable is Intent) {
                    ...
                    if (parcelable is PendingIntent) {
                        ...
                        val activityOptionsHidden = Refine.unsafeCast<ActivityOptionsHidden>(options).setCallerDisplayId(virtualDisplay.display.displayId)
    ...
    override fun onScreenOff() {
        // Jangan stop service saat layar mati kalau masih ada window aktif!
        mFreeformViews.removeAll { it.isDestroy }
        val autoClose = sp.getBoolean("auto_close_screen_off", false)
        if (autoClose) {
            mFreeformViews.forEach { it.destroy() }
            mFreeformViews.clear()
            stopSelf()
        }
        // Kalau tidak auto close, biarkan window tetap jalan
    }
    ...
    override fun onScreenOff() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            if (mFreeformViews.isEmpty()) stopSelf()
        } else {
            if (mFreeformViews.isEmpty()) stopSelf()
        }
    }
